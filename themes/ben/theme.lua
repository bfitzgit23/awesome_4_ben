...
-- Awesome WM Widgets
theme.widget_main_color 	= "#6a329f"
...
local mytextclock = wibox.widget.textclock(markup(white, "%a %h %d %l:%M %p"))
...
bbswidgettimer:connect_signal("timeout",
  function()
    ...
    if status:read() == nil then
        bbswidget:set_markup(markup(red, "AMD"))
    else
...
theme.weather = lain.widget.weather({
    ...
    city_id = 5161347, -- placeholder (not London)
...
function theme.at_screen_connect(s)
	...
	local names = { "MAIN", "GAMES", "EMERGE", "VM", "MUS" }
